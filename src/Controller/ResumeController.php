...
class ResumeController extends AbstractFOSRestController
{
     ...
     * @param Request $request
     * @param PublisherAMQP $publisherAMQP
     * @return Response
     */
    public function addCV(Request $request, PublisherAMQP $publisherAMQP): Response
    {
        $client = new DB($this->getParameter('app.db_url'));
        $collection = $client->selectCollection($this->getParameter('app.db_name'), $this->getParameter('app.db_collection'));
        try {
            ...
            $publisherAMQP->publishMessage(json_encode($request->toArray(), JSON_UNESCAPED_UNICODE));
            return $this->json([
